  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-4 py-10">
        ...
        {/* Main Card: Info Left + Image Right */}
          ...
          <div className="grid md:grid-cols-[340px_1fr]">
            ...
            <div className="p-6 md:p-8 border-b md:border-b-0 md:border-r border-border flex flex-col justify-between">
              ...
              <div>
                ...
                <div className="space-y-3 mb-6">
                  <div className="flex justify-between items-center">
                    <span className="text-muted-foreground text-sm">Bloc</span>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold">{apt.rooms} pièces</span>
                  </div>
                </div>

                <div className="border-t border-border pt-4 space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-muted-foreground text-sm">Surface Totale</span>
                    <span className="font-bold text-lg">{apt.surface} M2</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-muted-foreground text-sm">Prix TTC</span>
                    <span className="font-bold text-lg text-gold">{formatPrice(apt.prix)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-muted-foreground text-sm">Disponibilité</span>
                    <span className={`font-bold ${
                      apt.status === "Disponible" ? "text-available" :
                      apt.status === "Réservé" ? "text-reserved" : "text-sold"
                    }`}>{apt.status}</span>
                  </div>
                </div>
              </div>
              ...
              {apt.status === "Disponible" && (
                <a
                  href="#contact"
                  className="block text-center mt-6 px-6 py-3 bg-accent text-accent-foreground rounded-lg font-medium hover:opacity-90 transition-opacity"
                >
                  Nous contacter
                </a>
              )}
              ...
              <div className="grid grid-cols-3 gap-3 mt-4">
                {imageLabels.map((img, i) => (
                  ...
                  >
                    ...
                    <p className="text-[11px] text-center py-1 font-medium text-muted-foreground">
                      {img.label}
                    </p>
                  </button>
                ))}
